...
void testEnablePeripheralClockShouldEnableClockToUsartPeripheral(void) {
  ...
  TEST_ASSERT_EQUAL(((uint32)1) << USART0_CLOCK_ENABLE,
...
void testResetShouldResetUsartTransmitterAndReceiver(void) {
  ...
  Usart_Reset();
  TEST_ASSERT_EQUAL(AT91C_US_RSTRX | AT91C_US_RSTTX | AT91C_US_RXDIS |
                        AT91C_US_TXDIS,
...
void testSetBaudRateRegisterShouldSetUsartBaudRateRegisterToValuePassedAsParameter(
    void) {
