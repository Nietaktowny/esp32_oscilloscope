 ...
 * when you do that, CMOCK_MEM_SIZE is used for incremental size instead of
 * total */
...
/* this is used internally during pointer arithmetic. make sure this type is the
 * same size as the target's pointer type */
...
#define CMOCK_MEM_INDEX_SIZE                                                   \
  (CMOCK_MEM_INDEX_TYPE)(CMOCK_MEM_PTR_AS_INT)(                                \
      (sizeof(CMOCK_MEM_INDEX_TYPE) > CMOCK_MEM_ALIGN_SIZE)                    \
          ? sizeof(CMOCK_MEM_INDEX_TYPE)                                       \
