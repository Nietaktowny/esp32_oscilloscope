...
void setUp(void) { CMock_Guts_MemFreeAll(); }
...
void test_MemNewWillReturnNullIfGivenIllegalSizes(void) {
  ...
  TEST_ASSERT_EQUAL_HEX(
      CMOCK_GUTS_NONE,
      CMock_Guts_MemNew(CMOCK_MEM_SIZE - TEST_MEM_INDEX_SIZE + 1));
  TEST_ASSERT_NULL(CMock_Guts_GetAddressFor(CMOCK_GUTS_NONE));

  // verify we're cleared still
  TEST_ASSERT_LESS_OR_EQUAL_UINT32(CMOCK_MEM_SIZE,
...
void test_MemShouldProtectAgainstMemoryOverflow(void) {
  ...
  TEST_ASSERT_LESS_OR_EQUAL_UINT32(TEST_MEM_INDEX_SIZE,
                                   CMock_Guts_MemBytesFree());
  TEST_ASSERT_GREATER_OR_EQUAL_UINT32(CMOCK_MEM_SIZE,
...
void test_MemChainWillReturnNullAndDoNothingIfGivenIllegalInformation(void) {
  ...
  TEST_ASSERT_EQUAL_HEX(CMOCK_GUTS_NONE,
                        CMock_Guts_MemChain(next + CMOCK_MEM_SIZE, next));
  TEST_ASSERT_EQUAL_HEX(CMOCK_GUTS_NONE,
                        CMock_Guts_MemChain(next, next + CMOCK_MEM_SIZE));
...
void test_MemNextWillReturnNullIfGivenABadRoot(void) {
  ...
  TEST_ASSERT_EQUAL_HEX(CMOCK_GUTS_NONE,
                        CMock_Guts_MemNext(CMOCK_MEM_SIZE - 4));
...
void test_ThatWeCanClaimAndChainAFewElementsTogether(void) {
  ...
  TEST_ASSERT_MESSAGE(element[0] != CMOCK_GUTS_NONE,
                      "Should Not Have Returned CMOCK_GUTS_NONE");
  ...
  *((unsigned int *)CMock_Guts_GetAddressFor(element[0])) = 0;
  ...
  TEST_ASSERT_EQUAL(1 * (TEST_MEM_INDEX_SIZE + sizeof(unsigned int)),
                    CMock_Guts_MemBytesUsed());
  TEST_ASSERT_EQUAL(CMOCK_MEM_SIZE -
                        1 * (TEST_MEM_INDEX_SIZE + sizeof(unsigned int)),
                    CMock_Guts_MemBytesFree());

  // second element
  element[1] = CMock_Guts_MemNew(sizeof(unsigned int));
  TEST_ASSERT_MESSAGE(element[1] != CMOCK_GUTS_NONE,
                      "Should Not Have Returned CMOCK_GUTS_NONE");
  ...
  *((unsigned int *)CMock_Guts_GetAddressFor(element[1])) = 1;
  ...
  // verify we're using the right amount of memory
  TEST_ASSERT_EQUAL(2 * (TEST_MEM_INDEX_SIZE + sizeof(unsigned int)),
                    CMock_Guts_MemBytesUsed());
  TEST_ASSERT_EQUAL(CMOCK_MEM_SIZE -
                        2 * (TEST_MEM_INDEX_SIZE + sizeof(unsigned int)),
                    CMock_Guts_MemBytesFree());

  // third element
  element[2] = CMock_Guts_MemNew(sizeof(unsigned int));
  TEST_ASSERT_MESSAGE(element[2] != CMOCK_GUTS_NONE,
                      "Should Not Have Returned CMOCK_GUTS_NONE");
  ...
  *((unsigned int *)CMock_Guts_GetAddressFor(element[2])) = 2;
  ...
  // verify we're using the right amount of memory
  TEST_ASSERT_EQUAL(3 * (TEST_MEM_INDEX_SIZE + sizeof(unsigned int)),
                    CMock_Guts_MemBytesUsed());
  TEST_ASSERT_EQUAL(CMOCK_MEM_SIZE -
                        3 * (TEST_MEM_INDEX_SIZE + sizeof(unsigned int)),
                    CMock_Guts_MemBytesFree());

  // fourth element
  element[3] = CMock_Guts_MemNew(sizeof(unsigned int));
  TEST_ASSERT_MESSAGE(element[3] != CMOCK_GUTS_NONE,
                      "Should Not Have Returned CMOCK_GUTS_NONE");
  ...
  *((unsigned int *)CMock_Guts_GetAddressFor(element[3])) = 3;
  ...
  // verify we're using the right amount of memory
  TEST_ASSERT_EQUAL(4 * (TEST_MEM_INDEX_SIZE + sizeof(unsigned int)),
                    CMock_Guts_MemBytesUsed());
  TEST_ASSERT_EQUAL(CMOCK_MEM_SIZE -
                        4 * (TEST_MEM_INDEX_SIZE + sizeof(unsigned int)),
                    CMock_Guts_MemBytesFree());
  ...
  for (i = 0; i < 4; i++) {
    ...
    TEST_ASSERT_EQUAL(i,
                      *((unsigned int *)CMock_Guts_GetAddressFor(element[i])));
...
void test_ThatCMockStopsReturningMoreDataWhenItRunsOutOfMemory(void) {
  ...
  // CMOCK_MEM_ALIGN_SIZE bytes, therefore it will waste a byte each time so
  // each call will use (CMOCK_MEM_INDEX_SIZE + CMOCK_MEM_ALIGN_SIZE) bytes
  // (CMOCK_MEM_INDEX_SIZE for the index, 1 for the data, and
  // (CMOCK_MEM_ALIGN_SIZE - 1) wasted). therefore we can safely allocated
  // total/(CMOCK_MEM_INDEX_SIZE + CMOCK_MEM_ALIGN_SIZE) times.
  for (i = 0;
       i < (CMOCK_MEM_SIZE / (CMOCK_MEM_INDEX_SIZE + CMOCK_MEM_ALIGN_SIZE));
       i++) {
    TEST_ASSERT_EQUAL(i * (CMOCK_MEM_INDEX_SIZE + CMOCK_MEM_ALIGN_SIZE),
                      CMock_Guts_MemBytesUsed());
    TEST_ASSERT_EQUAL(CMOCK_MEM_SIZE -
                          i * (CMOCK_MEM_INDEX_SIZE + CMOCK_MEM_ALIGN_SIZE),
                      CMock_Guts_MemBytesFree());

    next = CMock_Guts_MemNew(1);
    TEST_ASSERT_MESSAGE(next != CMOCK_GUTS_NONE,
                        "Should Not Have Returned CMOCK_GUTS_NONE");

    first = CMock_Guts_MemChain(first, next);
    TEST_ASSERT_MESSAGE(first != CMOCK_GUTS_NONE,
                        "Should Not Have Returned CMOCK_GUTS_NONE");
  }
...
void test_ThatCMockStopsReturningMoreDataWhenAskForMoreThanItHasLeftEvenIfNotAtExactEnd(
    void) {
  unsigned int i;
  CMOCK_MEM_INDEX_TYPE first = CMOCK_GUTS_NONE;
  CMOCK_MEM_INDEX_TYPE next;

  // we're asking for (CMOCK_MEM_INDEX_SIZE + 8) bytes each time now
  // (CMOCK_MEM_INDEX_SIZE for index, 8 for data).
  // CMOCK_MEM_SIZE/(CMOCK_MEM_INDEX_SIZE + 8) requests will request as much
  // data as possible, while ensuring that there isn't enough memory for the
  // next request
  for (i = 0; i < CMOCK_MEM_SIZE / (CMOCK_MEM_INDEX_SIZE + 8); i++) {
    TEST_ASSERT_EQUAL(i * (CMOCK_MEM_INDEX_SIZE + 8),
                      CMock_Guts_MemBytesUsed());
    TEST_ASSERT_EQUAL(CMOCK_MEM_SIZE - i * (CMOCK_MEM_INDEX_SIZE + 8),
                      CMock_Guts_MemBytesFree());
  ...
  // verify we're at top of memory
  TEST_ASSERT_EQUAL(CMOCK_MEM_SIZE -
                        CMOCK_MEM_SIZE % (CMOCK_MEM_INDEX_SIZE + 8),
                    CMock_Guts_MemBytesUsed());
  TEST_ASSERT_EQUAL(CMOCK_MEM_SIZE % (CMOCK_MEM_INDEX_SIZE + 8),
                    CMock_Guts_MemBytesFree());
...
void test_ThatWeCanAskForAllSortsOfSizes(void) {
  ...
  TEST_IGNORE_MESSAGE(
      "Test relies on a particular environmental setup, which is not present");
#else
  ...
  TEST_ASSERT_EQUAL_HEX(CMOCK_GUTS_NONE,
                        CMock_Guts_MemNew(CMOCK_MEM_SIZE - sum + 8));
  TEST_ASSERT_EQUAL_HEX(CMOCK_GUTS_NONE,
                        CMock_Guts_MemNew(CMOCK_MEM_SIZE - sum + 1));
...
void test_MemEndOfChain(void) {
  ...
  element[0] = CMock_Guts_MemNew(sizeof(unsigned int));
  first = CMock_Guts_MemChain(first, element[0]);
  TEST_ASSERT_MESSAGE(element[0] == CMock_Guts_MemEndOfChain(first),
                      "Should have returned element[0]");

  // second element
  element[1] = CMock_Guts_MemNew(sizeof(unsigned int));
  CMock_Guts_MemChain(first, element[1]);
  TEST_ASSERT_MESSAGE(element[1] == CMock_Guts_MemEndOfChain(first),
                      "Should have returned element[1]");

  // third element
  element[2] = CMock_Guts_MemNew(sizeof(unsigned int));
  CMock_Guts_MemChain(first, element[2]);
  TEST_ASSERT_MESSAGE(element[2] == CMock_Guts_MemEndOfChain(first),
                      "Should have returned element[2]");

  // fourth element
  element[3] = CMock_Guts_MemNew(sizeof(unsigned int));
  CMock_Guts_MemChain(first, element[3]);
  TEST_ASSERT_MESSAGE(element[3] == CMock_Guts_MemEndOfChain(first),
                      "Should have returned element[3]");
