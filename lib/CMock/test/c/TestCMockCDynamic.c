...
#define TEST_MEM_INDEX_PAD                                                     \
  (CMOCK_MEM_INDEX_TYPE)(                                                      \
      ((CMOCK_MEM_INDEX_TYPE)sizeof(CMOCK_MEM_INDEX_TYPE) + 7) &               \
      ~7) // round up to nearest 4 byte boundary
...
void test_MemNewWillNowSupportSizesGreaterThanTheDefinesCMockSize(void) {
  ...
  TEST_ASSERT_MESSAGE(CMock_Guts_MemNew(CMOCK_MEM_SIZE - TEST_MEM_INDEX_SIZE +
                                        1) != CMOCK_GUTS_NONE,
                      "Should Not Have Returned CMOCK_GUTS_NONE");

  TEST_ASSERT_EQUAL(((CMOCK_MEM_INDEX_TYPE)CMOCK_MEM_SIZE + TEST_MEM_INDEX_PAD),
                    CMock_Guts_MemBytesUsed());
  TEST_ASSERT_EQUAL((CMOCK_MEM_INDEX_TYPE)CMOCK_MEM_SIZE,
...
void test_MemChainWillReturnNullAndDoNothingIfGivenIllegalInformation(void) {
  ...
  TEST_ASSERT_EQUAL(StartingSize - 8 - TEST_MEM_INDEX_PAD,
                    CMock_Guts_MemBytesFree());
...
void test_ThatWeCanClaimAndChainAFewElementsTogether(void) {
  ...
  *((unsigned int *)CMock_Guts_GetAddressFor(element[0])) = 0;
  ...
  TEST_ASSERT_EQUAL(StartingSize - 1 * (TEST_MEM_INDEX_PAD + 8),
                    CMock_Guts_MemBytesFree());
  ...
  *((unsigned int *)CMock_Guts_GetAddressFor(element[1])) = 1;
  ...
  TEST_ASSERT_EQUAL(StartingSize - 2 * (TEST_MEM_INDEX_PAD + 8),
                    CMock_Guts_MemBytesFree());
  ...
  *((unsigned int *)CMock_Guts_GetAddressFor(element[2])) = 2;
  ...
  TEST_ASSERT_EQUAL(StartingSize - 3 * (TEST_MEM_INDEX_PAD + 8),
                    CMock_Guts_MemBytesFree());
  ...
  *((unsigned int *)CMock_Guts_GetAddressFor(element[3])) = 3;
  ...
  TEST_ASSERT_EQUAL(StartingSize - 4 * (TEST_MEM_INDEX_PAD + 8),
                    CMock_Guts_MemBytesFree());
...
void test_ThatWeCanAskForAllSortsOfSizes(void) {
  ...
  CMOCK_MEM_INDEX_TYPE sizes_buffered[10] = {
      16, 16, 88, 16, 16, 40, 16, 920, 16, 88}; // includes counter
  ...
  for (i = 0; i < 10; i++) {
    ...
    cap = (StartingSize > (sum + CMOCK_MEM_SIZE)) ? StartingSize
                                                  : (sum + CMOCK_MEM_SIZE);
