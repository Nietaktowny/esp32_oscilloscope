...
/* CMock should handle UTF-8 characters in comments. The world is an awesomely
 * diverse place! */
/* my µC Rocks! Open Source, not ©! My language has no Ümlauts! ǺƜǝƧǾɱɛ! */ /**!
                                                                               Illegal: åäö */
...
VOID_TYPE_CRAZINESS void_type_craziness1(int, float, double, char, short, long,
...
char crazy_multiline(int a, unsigned int b);
...
incredible_descriptors(register const unsigned short a);

int32_t example_c99_type(int32_t param1);

void I2CIntRegister(uint32_t ui32Base, void (*pfnHandler)(void));

/* these are function pointers, not function declarations USING a function
 * pointer, and so should NOT get mocked */
