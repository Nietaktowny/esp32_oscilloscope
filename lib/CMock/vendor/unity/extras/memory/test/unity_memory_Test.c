...
void test_FreeNULLSafety(void) { free(NULL); }
...
/* This tricky set of defines lets us see if we are using the Spy, returns 1 if
 * true */
...
#if UNITY_OUTPUT_CHAR ==                                                       \
    UnityOutputCharSpy_OutputChar /* Works if no -Wundef -Werror */
...
void test_BufferOverrunFoundDuringFree(void) {
  ...
  UnityOutputCharSpy_Enable(0);
  Unity.CurrentTestFailed = 0;
  TEST_ASSERT_NOT_NULL(strstr(UnityOutputCharSpy_Get(),
...
#define TEST_ASSERT_MEMORY_ALL_FREE_LIFO_ORDER(first_mem_ptr, ptr)             \
  ptr = malloc(10);                                                            \
  free(ptr);                                                                   \
  TEST_ASSERT_EQUAL_PTR_MESSAGE(first_mem_ptr, ptr,                            \
                                "Memory was stranded, free in LIFO order");
