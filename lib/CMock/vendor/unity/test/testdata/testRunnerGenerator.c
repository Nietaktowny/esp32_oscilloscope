/* This Test File Is Used To Verify Many Combinations Of Using the Generate Test
 * Runner Script */
...
/* Notes about prefixes:
   test     - normal default prefix. these are "always run" tests for this
   procedure spec     - normal default prefix. required to run default
   setup/teardown calls. should   - normal default prefix. qwiktest - custom
   prefix for when tests skip all setup/teardown calls. custtest - custom prefix
   for when tests use custom setup/teardown calls. paratest - custom prefix for
   when we want to verify parameterized tests. extest   - custom prefix only
   used during cexception suitetest- custom prefix for when we want to use
   custom suite setup/teardown
*/
...
void tearDown(void) { CounterTeardown = 1; }

void custom_setup(void) { CounterSetup = 2; }

void custom_teardown(void) { CounterTeardown = 2; }
...
void test_ThisTestAlwaysPasses(void) { TEST_PASS(); }
...
void test_NotBeConfusedByLongComplicatedStrings(void) {
  const char *crazyString =
      ...
      "768c7149-c3fb-f704-71a2-63918d9195b2\r\nAccept: */*\r\nAccept-Encoding: "
      "gzip, deflate, sdch\r\nAccept-Language: "
      "en-GB,en-US;q=0.8,en;q=0.6\r\n\r\n";

  TEST_ASSERT_EQUAL_STRING_MESSAGE(crazyString, crazyString,
                                   "These Strings Are The Same");
}

/* The next test should still appear even though we have this confusing nested
 * comment thing going on http://looks_like_comments.com */
...
void test_StillNotBeConfusedByLongComplicatedStrings(void) {
  ...
  TEST_ASSERT_EQUAL_STRING_MESSAGE(crazyString, crazyString,
                                   "These Strings Are Still The Same");
}
...
void extest_ShouldHandleCExceptionInTest(void) {
  TEST_ASSERT_EQUAL_MESSAGE(1, CEXCEPTION_BEING_USED,
                            "Should be pulling in CException");
}
#endif
...
int main(void) { return custom_main(); }
#endif

void suitetest_ThisTestPassesWhenCustomSuiteSetupAndTeardownRan(void) {
  TEST_ASSERT_EQUAL_MESSAGE(1, CounterSuiteSetup,
