...
void testUnitySizeInitializationReminder(void) {
  /* This test ensures that sizeof(struct UNITY_STORAGE_T) doesn't change. If
   * this test breaks, go look at the initialization of the Unity global
   * variable in unity.c and make sure we're filling in the proper fields. */
  const char *message =
      "Unexpected size for UNITY_STORAGE_T struct. Please check that "
...
void testPreviousPass(void) { TEST_ASSERT_EQUAL_INT(0U, Unity.TestFailures); }
...
#if defined(UNITY_OUTPUT_FLUSH) &&                                             \
    defined(UNITY_OUTPUT_FLUSH_HEADER_DECLARATION)
...
/* ===================== THESE TEST WILL RUN IF YOUR CONFIG INCLUDES DETAIL
 * SUPPORT ================== */
...
void testThatDetailsCanBeHandleTwoDetails(void) {
  ...
  EXPECT_ABORT_BEGIN
  TEST_ASSERT_EQUAL_HEX8_MESSAGE(7, 8,
...
void testThatDetailsCanBeHandleSingleDetailClearingTwoDetails(void) {
  ...
  EXPECT_ABORT_BEGIN
  TEST_ASSERT_EQUAL_STRING_MESSAGE("MEH", "GUH",
                                   "Should Fail And Say DetailNew");
  VERIFY_FAILS_END
#endif
}
