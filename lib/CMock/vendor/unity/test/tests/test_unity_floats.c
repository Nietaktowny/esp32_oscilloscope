...
void testFloatPrinting(void) {
#if defined(UNITY_EXCLUDE_FLOAT_PRINT) || defined(UNITY_INCLUDE_DOUBLE) ||     \
  ...
  TEST_ASSERT_EQUAL_PRINT_FLOATING("9999999",
                                   9999999.0f); /*Last full print integer*/
  ...
  /* Some compilers have trouble with inexact float constants, a float cast
   * works generally */
...
static void printFloatValue(float f) {
  ...
  if (isfinite(lower))
    sprintf(expected_lower, "%.7g", lower);
  if (isfinite(higher))
    sprintf(expected_higher, "%.7g", higher);
  ...
  if (f < 1.0 || f > 10000000) {
    ...
    if (isfinite(lower2))
      sprintf(expected_lower2, "%.7g", lower2);
    if (isfinite(lower3))
      sprintf(expected_lower3, "%.7g", lower3);
    if (isfinite(higher2))
      sprintf(expected_higher2, "%.7g", higher2);
    if (isfinite(higher3))
      sprintf(expected_higher3, "%.7g", higher3);
  }
