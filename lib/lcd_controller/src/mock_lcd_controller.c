...
esp_err_t init_lcd(void)
{
  ...
  {
    esp_err_t cmock_cb_ret = Mock.init_lcd_CallbackFunctionPointer(Mock.init_lcd_CallbackCalls++);
    UNITY_CLR_DETAILS();
    return cmock_cb_ret;
  }
  UNITY_TEST_ASSERT_NOT_NULL(cmock_call_instance, cmock_line, CMockStringCalledMore);
  ...
  if (Mock.init_lcd_CallbackFunctionPointer != NULL)
  {
    cmock_call_instance->ReturnVal = Mock.init_lcd_CallbackFunctionPointer(Mock.init_lcd_CallbackCalls++);
  }
  UNITY_CLR_DETAILS();
  return cmock_call_instance->ReturnVal;
}
